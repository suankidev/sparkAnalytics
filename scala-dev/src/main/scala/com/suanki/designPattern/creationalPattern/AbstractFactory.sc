...
class Ec2Instance(capacity: String) extends Instance {
  ...
  override def attachStorage(storage: Storage): Unit = println(
    s"Storage is attached ${storage.getId}"
  )
}
...
/**   1. Hides factories as well as concrete object used from the client code Vs
  *      Hide the concrete object which are used from the client code.
  */

/** 2. suitable when mutlitple objects are designed to work together and client
  * must use products from single family at a time Vs concerned with one product
  * and it's subclasses collaboration of product it self with other object is
  * irrelevant
  */
