...
/** ?
  ...
  * code $ abstract it in seprate class
  */
...
case class UserWebDTO(name: String, address: String, age: String)
    extends UserDTO
...
class UserWebDTOBuilder extends UserDTOBuilder {
  ...
  override def withBirthday(date: LocalDate): UserWebDTOBuilder = {
    val ageInYear: java.time.Period =
      java.time.Period.between(date, java.time.LocalDate.now())
...
def createUser(): User = {
  ...
  val user: User =
    User(
      firstName = "Rajoo",
      lastName = "Singhania",
      birthdate = java.time.LocalDate.of(1992, 5, 23),
      address
    )
  user
}
