/** We have an existing object which provides the functionality that client
  * need, but client code can't use this object because it expects an object
...
/** but then there is another way where adapter form has a relation with
  * adapteer and implement other interface which client needs
  */
