...
class ComplexEnergyMeterOne(
  ...
  private[this] def wattConsumeInTotal_=(newVal: Double): Unit =
    _wattConsumedInTotal = newVal // setter
...
//  }
  ...
  def startMeasuring(): Unit = {
    ...
    turnDeviceOn()
    turnedOnAtMillis =
      System.currentTimeMillis() // milli seconds since jan 1970
...
object SomeComponent {
  ...
  def staticDispatch(device: Any): ComplexEnergyMeterOne = {
    if (device.isInstanceOf[LightBulb]) {
      ...
      new ComplexEnergyMeterOne(
        lightBulb.wattsPerSecond,
        lightBulb.turnOn,
        lightBulb.turnOff
      )
...
object InversionOfControlsThree extends App {
  ...
  /** *
    *
    * Every time we add new device we need to add pattern match(code in
    * SomeComponent for each type of device)
    */
}
