...
object Exercise {
  ...
  def VariablePattern(input: Any): Any = {
    ...
    val One = 1
    input match {
      case name @ (n: Boolean) /*name @ (_: Boolean)*/ =>
        s"matched a boolean  and ${n} "
...
//      case _: Boolean /*name @ (_: Boolean)*/          => s"matched a boolean in method2" // Boolean is matched
      ...
      case _: Int | _: String =>
        s"""isthis is int, isthis is string pattern match on two thing at same time"""
  ...
  def nonPrimitivePattern(input: Any): Any = {
    ...
    input match {
      case p @ Dog(n, a @ 3) =>
        s"""Matched ${p.getClass.getSimpleName} $n who is $a years old"""
      case p @ Dog(n, a) =>
        s"""Matched ${p.getClass.getSimpleName} $n who is $a years old"""
      case person: Person if person.isAdult => s"Guard: ${person.getName}"
      case student @ Student(n, a) if a > 18 =>
        s"""student: name: ${n}, adult age: ${a}"""
      case student @ Student(n, a) if a < 18 =>
        s"""student: name: ${n}, adult age: ${a}"""
