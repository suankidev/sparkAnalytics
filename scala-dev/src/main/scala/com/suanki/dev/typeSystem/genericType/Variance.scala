...
import com.suanki.utilities.logger.StdOut._
...
/**   1. what we want is for FoodBowl[Apple] to be subtype of FoodBowl[Fruit]
  *      and FoodBowl[Food] 2. This relationship is known as varaince and is
  *      demarked by + and- on the generic 3. [+F]mean theat the type parameter
  *      is co-variant 4. [-F] mean that the type parameter is contra-variant 5.
  *      if if a type parameter has neither +or - , it is considered invariant
  */
...
class Variance {
  ...
  def code: Unit = {
    ...
    /** Note: com.suanki.dev.typeSystem.genericType.Apple <:
      * com.suanki.dev.typeSystem.genericType.Food, but class FoodBowl1 is
      * invariant in type F. [error] You may wish to define F as +F instead.
      * (SLS 4.5)
      */
    // 1.
