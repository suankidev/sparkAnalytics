...
class Module03Solutions extends AbstractSuit {
  ...
  // UNCOMMENT BELOW
  test(
    "Create a new Complex number and check the values for the real/imaginary parts"
  ) {
  ...
  test(
    "Create a new complex number with double values and check those values"
  ) {
  ...
  test(
    "Create a complex number from a real number, imaginary part should be 0"
  ) {
