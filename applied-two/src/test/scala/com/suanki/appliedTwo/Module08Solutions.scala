...
class Module08Solutions extends AbstractSuit {
  ...
  test("Fun with top types of Strings") {
    ...
    mAndMsRef.isInstanceOf[String] should be(
      true
    ) // why? Because this is a runtime test!
  ...
  test("Fun with top types of values") {
    ...
    lots.isInstanceOf[AnyRef] should be(
      true
    ) // huh? Why? - because it is autoboxed in order to give it instanceOf implicitly :-)
  ...
  test("make a list from any product") {
    val tup = (1, '2', "three")
