...
class Module13Solutions
  ...
  test("Convert a string to a list of chars") {
    stringToChars(seq1) should be(
      List('G', 'T', 'A', 'A', 'G', 'C', 'T', 'T', 'A', 'C')
    )
  }
  ...
  // into a set
  def stringToSetOfChars(s1: String): List[Set[Char]] = s1.toList.map { c =>
    Set(c)
  }

  test(
    "Convert a string to a list of set of chars (each set with one char in it)"
  ) {
    stringToSetOfChars(seq1).toString should be(
      "List(Set(G), Set(T), Set(A), Set(A), Set(G), Set(C), Set(T), Set(T), Set(A), Set(C))"
    )
  }
  ...
  test("Combine list of sets with list of new chars") {
    ...
    finalSet.toString should be(
      "List(Set(G), Set(T, A, -), Set(A, C), Set(A), Set(G, C), Set(C), Set(T), Set(T, -, A), Set(A), Set(C))"
    )
  }
