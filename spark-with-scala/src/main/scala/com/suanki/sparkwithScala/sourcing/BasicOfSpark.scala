...
class BasicOfSpark(spark: SparkSession, util: CommonUtils) {
  ...
  def dataFrames(): Unit = {
    ...
    /** root \|-- Acceleration: double (nullable = true) \|-- Cylinders: long
      * (nullable = true) \|-- Displacement: double (nullable = true) \|--
      * Horsepower: long (nullable \= true) \|-- Miles_per_Gallon: double
      * (nullable = true) \|-- Name: string (nullable = true) \|-- Origin:
      * string (nullable = true) \|-- Weight_in_lbs: long (nullable \= true)
...
//    allretailer.where(col("inv_quantity_on_hand") > 500 ).count()
  ...
  def dfReadMode(): Unit = {
    ...
    /** * ->Drop the corrupted records ->Handle the corrupted records and store
      * them a separate location ->Fail the job if we get corrupted record
      */
