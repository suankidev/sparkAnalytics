...
class WeiredFileSourcing(spark:SparkSession) { self => this
  ...
  def run():Unit={
    ...
    val filedf = {
      spark.read.format("csv").option("header", value = false).option("sep", ",")
        .option("emptyValue", "")
   ...
   val splittedArrayString = {
     fileRddWithIndex.map(row => row._1).map(x => {
    ...
    val finalRdd = {
      splittedArrayString.map({
        case row => Row.fromSeq(
          row.toSeq
          .map(x => {
