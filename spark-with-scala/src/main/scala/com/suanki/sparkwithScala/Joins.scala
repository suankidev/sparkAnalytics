...
class Joins(spark: SparkSession) {
  ...
  def innerJoin(): Unit = {
    ...
    val innerJoinDF =
      empDF.join(deptDF, joinExprs = joinExprs, joinType = "inner")
    ...
    val leftSemi =
      empDF.join(deptDF, joinExprs = joinExprs, joinType = "leftsemi")
    val leftAnti =
      empDF.join(deptDF, joinExprs = joinExprs, joinType = "leftanti")
